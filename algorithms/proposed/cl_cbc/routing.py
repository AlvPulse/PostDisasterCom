...
class CLCBCRouting(RoutingAlgorithm):
    ...
    def decide(self):
        ...
        for s_uav in self.service_uavs:
            ...
            for r_uav in self.relay_uavs:
                ...
                # Use Relay Selection logic which integrates capacity, queue length, and energy
                from .relay_selection import CLCBCRelaySelection

                q_relay = r_uav.queue.total_bits
                energy_rem = getattr(r_uav, 'energy_remaining', 1.0)

                score = CLCBCRelaySelection.get_relay_score(cap, q_relay, energy_rem)
